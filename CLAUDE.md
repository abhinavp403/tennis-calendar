...
**`race.json`**: `{atp: {asOf, players: RacePlayer[]}, wta: {...}}` — the current top 10 in the season race (ATP Race to Turin / Race to the WTA Finals), each with `rank`, `name`, `full`, `country`, `points`, `tournaments`, `titles`, `qualified`, and optional `results: [{event, round, points, group}]` — the per-event breakdown (`group` is `slam` / `masters` / `other`, from the table's top header colspans; `round: "A"` means skipped a mandatory event). `results` is only stored when a row's cell count matches the header's columns, so a layout change drops the breakdown rather than misfiling results; the per-event points always sum to `points`. Written by `scripts/fetchRace.js` from the singles "Points breakdown" table on Wikipedia's `"<year> ATP Finals"` / `"<year> WTA Finals"` pages; only the latest snapshot is kept (the race resets each January). It refuses to overwrite with a partial parse. Shown in `RaceToFinalsDialog.jsx` with the qualification cutoff after #8. Optional in the renderer — if missing, the dialog shows an empty state.
...
- **`end` is that tour's *singles* final day** — which is why `fixDates.js` reads only the per-tour singles subpage (`"<year> <Event> – Women's singles"` / `"– Men's singles"` / `"– Singles"`) and never the parent article's date range. The parent range ends on the **doubles** final (2026 Berlin reads "15–22 June"; singles was the 21st) and, on a **combined ATP/WTA** page, on the **men's** final (2026 Miami: WTA Mar 28, ATP Mar 29) — so using it pushes dates a day late. Roughly 86% of subpages carry no infobox date, so most tournaments can't be auto-verified; the script logs "No singles final date … leaving X alone" and makes no change. That's deliberate: it runs unattended twice a day, so a wrong correction is worse than none. Fix those by hand in the Gist (as was done for Adelaide, Eastbourne ATP, Iași and the SP Open).
